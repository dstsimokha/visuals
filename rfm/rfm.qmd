---
title: "RFM"
subtitle: "Quarto (RevealJS) & Plotly presentation"
...
df = pd.DataFrame({
    ...
    'recency': np.random.choice(5, 500_000, p=[.05, .1, .5, .3, .05])+1,
    'frequency': np.random.choice(5, 500_000, p=[.05, .3, .5, .1, .05])+1,
...
fig.update_layout(
    ...
    margin=dict(l=0, r=0, b=0, t=15, pad=5),
    font=dict(family="Poppins, sans-serif", size=16),
    paper_bgcolor='white', plot_bgcolor='white',
    xaxis=dict(showline = False, ticks = '', title = 'Recency'),
    yaxis=dict(showline = False, ticks = '', title = 'Frequency')
...
rfm_regions = fig
rfm_regions.show(config=config)
```
